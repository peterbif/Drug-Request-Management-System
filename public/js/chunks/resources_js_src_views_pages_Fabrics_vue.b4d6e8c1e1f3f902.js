  ...
  methods: {
        ...
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_2___default().wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                _context.next = 2;
                return axios__WEBPACK_IMPORTED_MODULE_3___default().get('api/getallFabrics' + '?token=' + _this2.$store.state.token).then(function (response) {
                  _this2.allFabrics = response.data.allFabrics;
                  _this2.allFabrics2 = response.data.allFabrics2;
                  _this2.totalFabrics = response.data.totalFabrics;
        ...
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_2___default().wrap(function _callee2$(_context2) {
          while (1) {
            switch (_context2.prev = _context2.next) {
              case 0:
                query = _this3.search;
                _this3.form.post('/api/search/Fabrics')
                // .get('api/auth/findDesignation?q='+ query + '&token=' + this.$store.state.token )
                .then(function (response) {
                  _this3.allFabrics = response.data.allFabrics;
                  _this3.allFabrics2 = response.data.allFabrics2;
                  _this3.totalFabricss = response.data.totalFabricss;
                  // this.amounts = response.data.amount;
                })["catch"](function () {
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                ...
                _context3.next = 3;
                return _this4.form.post('api/fabrics' + '?token=' + _this4.$store.state.token).then(function () {
                  _this4.$toastr.success('Fabrics created successfully', 'Success');
                  _this4.dialog = false;
                  Fire.$emit('createFabricsEvent');
                  _this4.form.reset();
                })["catch"](function (error) {
                  _this4.progress = true;
                  _this4.$toastr.error('Can not create fabrics', error);
                });
              case 3:
              case "end":
    ...
    updateFabrics: function updateFabrics(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          _this5.form.put('api/fabrics/' + _this5.form.id + '?token=' + _this5.$store.state.token).then(function () {
            _this5.$toastr.success('Fabrics updated successfully', 'Success');
            Fire.$emit('updatedFabricsEvent'); //update an event
            _this5.dialog = false;
          })["catch"](function (error) {
            _this5.btn = 'Updating Fabrics';
            _this5.btn2 = '';
            _this5.$toastr.error('Can not update Fabrics details', 'Error');
          });
        }
      });
    },
    deleteFabrics: function deleteFabrics(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          _this6.form["delete"]('api/fabrics/' + item.id + '?token=' + _this6.$store.state.token).then(function () {
