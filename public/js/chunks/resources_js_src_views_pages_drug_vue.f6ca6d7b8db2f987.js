...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    ...
    submitImportedFile: function submitImportedFile() {
      ...
      return (0,_var_www_html_pharmacy_node_modules_babel_runtime_helpers_esm_asyncToGenerator__WEBPACK_IMPORTED_MODULE_0__["default"])( /*#__PURE__*/_babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_1___default().mark(function _callee4() {
        var drugDetails, formData;
        return _babel_runtime_regenerator__WEBPACK_IMPORTED_MODULE_1___default().wrap(function _callee4$(_context4) {
          while (1) {
            switch (_context4.prev = _context4.next) {
              case 0:
                drugDetails = document.querySelector('#drugs'); // alert(drugDetails);
                formData = new FormData(drugDetails); // formData.append('file', this.$refs.file.files[0]);
                //  await axios.post('/api/importFile', {formData: formData}).then(response => {
                _this7.form.post('/api/importFile' + '?token=' + _this7.$store.state.token).then(function () {
                  _this7.$toastr.success('Drugs uploaded', 'Success');
                  _this7.dialog3 = false;
                  Fire.$emit("fileUploaded");
                  _this7.file = "";
                })["catch"](function (error) {
                  _this7.progress = true;
                  _this7.$toastr.error('Can not upload file', error);
                });
              case 3:
              case "end":
  ...
  created: function created() {
    ...
    Fire.$on(["createDrugEvent", "DeleteDrugEvent", "updatedDrugEvent", "fileUploaded"], function () {
      //listen to  events
      _this11.getAllDrugs();
    });
  }
