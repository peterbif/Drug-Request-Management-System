  ...
  methods: {
            ...
            switch (_context2.prev = _context2.next) {
              case 0:
                ...
                return axios__WEBPACK_IMPORTED_MODULE_2___default().get('/api/search/staffCategory?s=' + query).then(function (response) {
                  ...
                  _this.$toastr.error("search failed", "Error");
                });
              case 3:
              case "end":
          ...
          while (1) {
            switch (_context3.prev = _context3.next) {
              case 0:
                ...
                _context3.next = 3;
                return _this2.form.post('/api/staffCategory' + '?token=' + _this2.$store.state.token).then(function () {
                  _this2.$toastr.success('Account created successfully', 'Success');
                  _this2.dialog = false;
                  Fire.$emit("createdStaffEvent");
                  _this2.form.reset();
                })["catch"](function (error) {
                  _this2.progress = true;
                  _this2.$toastr.error('Can not create an Account', error);
                });
              case 3:
              case "end":
    ...
    updateStaff: function updateStaff(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          ...
          _this3.spinnerLoading = true;
          _this3.form.put("api/staffCategory/" + _this3.form.id + '?token=' + _this3.$store.state.token).then(function () {
            _this3.$toastr.success("Staff updated successfully", "Success");
            Fire.$emit("updatedStaffEvent"); //update an event
            _this3.dialog = false;
          })["catch"](function (error) {
            _this3.btn = "Updating Staff";
            _this3.btn2 = "";
            _this3.$toastr.error("Can not update Staff information", "Error");
          });
        }
      });
    },
    deleteStaff: function deleteStaff(item) {
      ...
      Swal.fire({
        ...
        if (result.value) {
          _this4.form["delete"]("api/staffCategory/" + item.id + '?token=' + _this4.$store.state.token).then(function () {
